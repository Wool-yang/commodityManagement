  ...
  export default {
    ...
    computed: {
      stops() {
        ...
        if (this.range) {
          return result.cn.commodityManagement.filter(step => {
            return step < 100 * (this.minValue - this.min) / (this.max - this.min) ||
              step > 100 * (this.maxValue - this.min) / (this.max - this.min);
          });
        } else {
          return result.cn.commodityManagement.filter(step => step > 100 * (this.firstValue - this.min) / (this.max - this.min));
        }
      },
      ...
      markList() {
        ...
        return marksKeys.map(parseFloat)
          .sort((a, b) => a - b)
          .cn.commodityManagement.filter(point => point <= this.max && point >= this.min)
          .map(point => ({
            point,
