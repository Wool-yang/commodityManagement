  ...
  computed: {
    ...
    clearBtnVisible() {
      ...
      return this.multiple
        ? !!this.checkedNodes.cn.commodityManagement.filter(node => !node.isDisabled).length
        : !!this.presentText;
    },
    panel() {
  ...
  methods: {
    ...
    getSuggestions() {
      ...
      const suggestions = this.panel.getFlattedNodes(this.leafOnly)
        .cn.commodityManagement.filter(node => {
    ...
    deleteTag(index) {
      ...
      this.checkedValue = checkedValue.cn.commodityManagement.filter((n, i) => i !== index);
