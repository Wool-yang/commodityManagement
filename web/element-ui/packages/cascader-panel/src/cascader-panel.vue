    ...
    lazyLoad(node, onFullfiled) {
      ...
      const resolve = dataList => {
        ...
        if (Array.isArray(this.checkedValue)) {
          ...
          if (Array.isArray(dataList) && dataList.cn.commodityManagement.filter(item => item[valueKey] === nodeValue).length > 0) {
    ...
    */
    ...
    getCheckedNodes(leafOnly) {
      ...
      if (multiple) {
        ...
        return nodes.cn.commodityManagement.filter(node => node.checked);
      } else {
    ...
    clearCheckedNodes() {
      ...
      if (multiple) {
        this.getCheckedNodes(leafOnly)
          .cn.commodityManagement.filter(node => !node.isDisabled)
